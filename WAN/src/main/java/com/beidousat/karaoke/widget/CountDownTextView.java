...
public class CountDownTextView extends TextView {
    ...
    private int mMaxTimerSec = BnsConfig.CNT_FINISH; //单位秒
    ...
    public CountDownTextView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        a.getInt(R.styleable.CountDownTextView_maxCountDownSec, mMaxTimerSec);
        a.recycle();
    ...
    private void init() {
        //加2s是为了能显示出30s和0s
        mTimer = new CountDownTimer((mMaxTimerSec +2)*1000, 1000) {
            @Override
        ...
        setText(mMaxTimerSec +" s");
    }

    public void setMaxTimerSec(int sec) {
        //保证最低分享时间为30s
        mMaxTimerSec = sec < mMaxTimerSec ? mMaxTimerSec : sec;
        if (mTimer != null) {
            mTimer.cancel();
        }
        init();
