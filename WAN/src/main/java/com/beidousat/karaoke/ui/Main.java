...
public class Main extends BaseActivity implements View.OnClickListener,
    ...
    private TextView mTvProgress,mTvTips,lable;
    private View mRoot;
...
//                break;
    ...
    private void initPlayer() {
        ...
        if (mPresentation != null) {
            ...
            player = new OriginPlayer(mPresentation.getSurfaceView(), main_surf,Main.this, hdmiWidth, hdmiHeight);
...
//            mMarqueePlayer.setVisibility(View.VISIBLE);
    ...
    private void restoreUserInfo() {
        ...
        if (currenttime - lasttime > Common.timelimit) {
            BoughtMeal.getInstance().clearMealInfoSharePreference();
        }
        BoughtMeal.getInstance().restoreMealInfoFromSharePreference();
        mQureyHelper.queryUser().post();
...
//        IntentFilter filter = new IntentFilter();
    ...
    private void checkUsbKey() {
        ...
        boolean isStore = com.beidousat.libbns.util.UsbFileUtil.isUsbExitBoxKey();
        File updateFile=com.beidousat.libbns.util.UsbFileUtil.isUsbExitUpdataApp();
        if (isStore) {
            PrefData.setPassword(this, "666888");
            Toast.makeText(this, "重置机顶盒密码完成", Toast.LENGTH_LONG).show();
        }
         if(com.beidousat.libbns.util.UsbFileUtil.isUsbExitUpdataApp().exists()){
            int mType=18;
            if (updateFile != null && updateFile.exists()) {
                    ...
                    if (mType == 20) { //H8
                        PackageUtil.installApkH8(this, updateFile, className);
                    } else if (18 == mType) {//晨芯3288
                        PackageUtil.installApk(this, updateFile, className);
                    } else if (24 == mType) {//音诺恒-3288
                        PackageUtil.installApkFor901(this, updateFile, className);
                    }
            }
        }
    }
