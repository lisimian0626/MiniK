...
public class HttpGetProxy {
    ...
    public HttpGetProxy() {
        try {
            ...
            Logger.w(TAG, "HttpGetProxy ex:" + e.toString());
        }
    }
    ...
    private void startProxy() {
        while (true) {
            ...
            try {
                ...
                new Thread() {
                    public void run() {
                        try {
                            ...
                            Logger.e(TAG, "startProxy Thread ex:" + e.toString());
                        }
                ...
                Logger.e(TAG, "startProxy IOException ex:" + e.toString());
            }
        }
    }
                ...
                while (true) {
                    synchronized (this) {
                        if (!sentResponseHeader) {
                            ...
                            if (cachebuf != null) {
                                Logger.d(TAG, "downedLen:" + cachebuf.downedLen);
                                int cpylen = cachebuf.downedLen - start;
                                byte[] buf = new byte[cpylen];
                                Logger.d(TAG, "downedLen:" + cachebuf.downedLen + " start : " + start + " cpylen " + cpylen);
...
//                ex.printStackTrace();
                Logger.w(TAG, "run ex:" + ex.toString());
            } finally {
                closeSockets();
            }
        }
