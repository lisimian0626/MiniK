...
public class HttpGetProxyUtils {
    ...
    public long sendPrebufferToMP(String fileName, long range) {
        ...
        if (file.exists() == false) {
            Logger.i(TAG, ">>>不存在预加载文件");
            return 0;
        }
        if (range > (file.length())) {// Range大小超过预缓存的太小
            Logger.i(TAG, ">>>不读取预加载文件 range:" + range + ",buffer:" + file.length());
            return 0;
        }

        if (file.length() < MIN_SIZE) {// 可用的预缓存太小，没必要读取以及重发Request
            Logger.i(TAG, ">>>预加载文件太小，不读取预加载");
            return 0;
        }
...
//				fInputStream.skip(range);
            ...
            Logger.i(TAG, ">>>读取预加载耗时:" + costTime);
            Logger.i(TAG, ">>>读取完毕...下载:" + file.length() + ",读取:" + fileBufferSize);
