...
public class ChooseSongs {
    ...
    private List<Song> mSongHasSung = new ArrayList<Song>();
        ...
        if (!TextUtils.isEmpty(string) && !"[]".equals(string)) {
            try {
                ...
                if (songs != null && songs.size() > 0) {
                    ...
                    EventBusUtil.postSticky(EventBusId.id.PLAYER_PLAY_SONG, mSongInfos.get(0));
                }
            } catch (Exception e) {
    ...
    public boolean addSong(final Song songInfo) {
        try {
            if (canAddSong()) {
                if (mSongInfos.isEmpty() && BoughtMeal.getInstance().isBuySong()) {//包曲第一首播放提示
                    mPromptDialog = new PromptDialog(Main.mMainActivity);
    ...
    public boolean add2Top(Song songInfo) {
        try {
            if (canAddSong()) {
    ...
    public void add2Sungs(final Song songInfo) {
        try {
            ...
            new Handler().postDelayed(new Runnable() {//延时，避免录音文件没写完就上传
                @Override
                public void run() {
                    Meal meal = BoughtMeal.getInstance().getMeal();
                    String orderSn = "";
                    if (meal != null) {
                        orderSn = meal.getOrderSn();
                    }
                    RecordFileUploader.getInstance().addFile(AudioRecordFileUtil.getRecordFile(songInfo.RecordFile).getAbsolutePath(),
                            orderSn, songInfo.ID, songInfo.SimpName, songInfo.SingerName, songInfo.score);
                }
            }, 1000);
        } catch (Exception e) {
