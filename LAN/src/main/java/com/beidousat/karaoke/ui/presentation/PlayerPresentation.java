...
import com.beidousat.karaoke.ad.AdStartEndGetter;
...
import com.beidousat.karaoke.widget.ReceiveMicData;
import com.beidousat.karaoke.widget.VoiceDataSources;
import com.beidousat.karaoke.widget.VoiceGradeView;
...
public class PlayerPresentation extends Presentation implements AdsRequestListener {
    ...
    private static final String TAG = PlayerPresentation.class.getSimpleName();
    ...
    private RecyclerImageView mIvAdPasue, mIvAdLast;
...
//    }
    ...
    public void showPauseAd(boolean show, boolean isMovie) {
        ...
        if (show && !mIvAdPasue.isShown()) {
            ...
            if (isMovie) {
                mAdPauseGetter.getMovieStopAd(song != null ? song.ID : "");
            } else {
                mAdPauseGetter.getSongStopAd(song != null ? song.ID : "");
            }
    ...
    public void showLast5SecAd(boolean show) {
        if (show && !mIvAdLast.isShown()) {
            Song song = ChooseSongs.getInstance(getContext().getApplicationContext()).getFirstSong();
            mAdEndGetter.getEnd(song != null ? song.ID : "");
    ...
    private void showAds(final boolean show) {
        ...
        if (show) {
            if (mAdCorner != null) {
                ...
                Song song = ChooseSongs.getInstance(getContext().getApplicationContext()).getFirstSong();
                if (song != null) {
                    mCornerGetter.getCorner("J1", song.ID);
                } else {
                    mCornerGetter.getCorner("J1");
                }
            }
        } else {
    ...
    private void showCorner() {
        ...
        mAdBillHelper.billAd(mAdCorner, "J1");
    }
    ...
    public void showScoreView(int mode) {
        boolean show = mode != 0;
        findViewById(R.id.fl_score_bg).setVisibility(show ? View.VISIBLE : View.GONE);
        voiceGradeView.setVisibility(show ? View.VISIBLE : View.GONE);
        mTvScore.setVisibility(show ? View.VISIBLE : View.GONE);
        mTvScoreMode.setVisibility(show ? View.VISIBLE : View.GONE);
        mTvScoreMode.setText(mode == 1 ? R.string.mode_normal : R.string.professional);
    }
    ...
    private AdsRequestListener mAdPauseListener = new AdsRequestListener() {
        ...
        public void onAdsRequestSuccess(Ad ad) {
            if (ad != null && !TextUtils.isEmpty(ad.ADContent)) {
                ...
                mAdBillHelper.billAd(ad, mIsPlayingMoving ? "S2" : "S1");
            }
        }
    ...
    public void showStartAd(boolean show) {
        if (show && !mIvAdLast.isShown()) {
            Song song = ChooseSongs.getInstance(getContext().getApplicationContext()).getFirstSong();
            mAdStartGetter.getStart(song != null ? song.ID : "");
        } else if (!show && mIvAdLast.isShown()) {
            mIvAdLast.setVisibility(View.GONE);
        }
    }
