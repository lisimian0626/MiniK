...
public class MyDownloader {
    ...
    private FileDownloadListener mFinishListener = new FileDownloadLargeFileListener() {
        ...
        protected void pending(BaseDownloadTask task, long soFarBytes, long totalBytes) {
            ...
            //检测磁盘空间是否充足
            if (!StorageUtil.isUsbDiskHasAvailableSpace(task.getSmallFileTotalBytes() - task.getSmallFileSoFarBytes())) {
    ...
    private void addSongToSongCache(String path) {
        ...
        songDao.insertSong(DiskFileUtil.convertDiskpathToServerPath(path), 0);
    }

    /**
     * 删除磁盘文件以及数据库中缓存记录;只删除最少点播的10首
     */
