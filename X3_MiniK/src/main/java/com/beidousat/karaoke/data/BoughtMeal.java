...
public class BoughtMeal extends Observable {
    ...
    public void notifyMealObervers() {
        setChanged();
        notifyObservers();
    }
    ...
    public void setSong(int count){
        if (getMealType() == Meal.SONG) {
            PreferenceUtil.setInt(LanApp.getInstance(), PREF_KEY_LEFT_SONGS, count);
            restoreMealInfoFromSharePreference();
        }
    }
